...
def train_agent(train_params, train_env_params, eval_env_params, obs_params):
    ...
    state_size = n_features_per_node * n_nodes + RLObs.additional_dim
    ...
    # policy.qnetwork_local = torch.load("./checkpoints/multi-sp-231014154949-2400.pth")
    ...
    for episode_idx in range(n_episodes):
        ...
        max_steps = int(train_env._max_episode_steps)
        ...
        for step in range(max_steps):
            ...
            for agent in train_env.get_agent_handles():
                if info['action_required'][agent]:
                    ...
                    # if agent_obs[agent][231] == 4:
                    #     action = 4
                    # else:
                    #     action = int(agent_obs[agent][231])
        ...
        if episode_idx % checkpoint_interval == 0:
            torch.save(policy.qnetwork_local, './checkpoints/gpu' + training_id + '-' + str(episode_idx) + '.pth')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate", help="learning rate", default=0.5e-3, type=float)
    ...
    parser.add_argument("--num_threads", help="number of threads PyTorch can use", default=6, type=int)
