...
def _split_node_into_feature_groups(node) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    data[3] = node.potential_conflict_dist
...
def _split_subtree_into_feature_groups(node, current_tree_depth: int, max_tree_depth: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for direction in TreeObsForRailEnv.tree_explored_actions_char:
        sub_data, sub_distance, sub_agent_data = _split_subtree_into_feature_groups(node.childs[direction], current_tree_depth + 1,
                                                                                    max_tree_depth)
...
def normalize_observation(observation, tree_depth: int, observation_radius=0):
    ...
    """
    # observation[tree, one_hot_action,delay]
    data, distance, agent_data = split_tree_into_feature_groups(observation[0], tree_depth)

    data = norm_obs_clip(data, fixed_radius=observation_radius)
    distance = norm_obs_clip(distance, normalize_to_range=True)
    agent_data = np.clip(agent_data, -1, 1)
    a = np.reshape(observation[1], 1)
    b = np.array(int(observation[2]))
    b= np.reshape(b,1)
    normalized_obs = np.concatenate((np.concatenate((data, distance)), agent_data, a, b))
